...
<div class="card">
    ...
    <ul class="category-list">
        <?php foreach ($categorias as $c): ?>
            <li ondblclick="verCategoria(<?= $c['id'] ?>)">
                <?= $c['nombre'] ?>
            </li>
        <?php endforeach; ?>
    </ul>
...
.category-list li:hover {
    background: rgba(46,255,161,0.12);
    transform: scale(1.05);
    box-shadow: 0 0 15px rgba(46,255,161,0.3);
}
...
function verCategoria(id){
    window.location.href = "prompts_categoria.php?id=" + id;
}
</script>
