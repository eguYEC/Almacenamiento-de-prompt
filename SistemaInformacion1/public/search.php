...
<div class="card">
    <div class="card-header">
        <h2>Buscar Prompts</h2>

        <div class="filters">
            <input type="text" id="searchInput" placeholder="🔍 Buscar por palabra...">
    ...
    <table class="table">
        ...
        <?php foreach ($prompts as $p): ?>
            <tr class="prompt-row"
                data-title="<?= strtolower($p['titulo_contenido']) ?>"
                data-content="<?= strtolower($p['contenido']) ?>"
                data-ia="<?= $p['ia_destino'] ?>">

                <td>
                    <a href="#" onclick="openModal(<?= htmlspecialchars(json_encode($p)) ?>)">
                        <?= $p['titulo_contenido'] ?>
                    </a>
                </td>
                ...
                <td>
                    <button class="open-ai-btn"
                        onclick="openInAI(`<?= htmlspecialchars(addslashes($p['contenido'])) ?>`, '<?= $p['ia_destino'] ?>')">
                        🚀 Abrir
                    </button>
                </td>

                <td>
                    <span class="star <?= $p['idFavorito'] ? 'active' : '' ?>"
                        onclick="toggleFavorite(<?= $p['id'] ?>, this)">★</span>
                </td>
                <?php if ($isAdmin): ?>
                <td>
                    <form action="delete_prompt.php" method="POST" onsubmit="return confirm('¿Seguro que deseas eliminar este prompt?');">
                        <input type="hidden" name="idPrompt" value="<?= $p['id'] ?>">
...
// Toggle favorito
function toggleFavorite(idPrompt, element) {
    fetch('toggle_favorite.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded'
        },
        body: 'idPrompt=' + idPrompt
    })
    .then(res => res.text())
    .then(data => {
...
function filterPrompts(){
    ...
    rows.forEach(row=>{
        ...
        const rowIA = row.dataset.ia;

        const matchText = title.includes(text) || content.includes(text);
        const matchIA = ia === "" || rowIA === ia;

        row.style.display = (matchText && matchIA) ? "" : "none";
    });
}

searchInput.addEventListener("keyup", filterPrompts);
iaFilter.addEventListener("change", filterPrompts);


// ================= ABRIR EN IA + COPIAR =================
function openInAI(prompt, ia){
