...
$id = $_GET['id'] ?? null;
if (!$id) {
    header("Location: edit_prompts.php");
    exit;
}

$stmt = $conn->prepare("
    SELECT p.titulo_contenido, p.descripcion, p.ia_destino, v.contenido
    FROM Prompt p
    LEFT JOIN Version v ON p.id = v.id_prompt AND v.numero = p.version_actual
    WHERE p.id = ?
");
$stmt->execute([$id]);
...
<div class="page-center">
    ...
    <div class="card">
        ...
        <div class="card-header">
            <h2>Editar Prompt</h2>
            <a href="edit.php" class="volver-btn">Volver</a>
        </div>

        <form action="save_prompt.php" method="POST" class="form-edit">

            <input type="hidden" name="id_prompt" value="<?= $id ?>">

            <label>Título</label>
            <input type="text" name="titulo" value="<?= htmlspecialchars($p['titulo_contenido']) ?>" required>

            <label>Descripción</label>
            <textarea name="descripcion" rows="3" required><?= htmlspecialchars($p['descripcion']) ?></textarea>

            <label>IA</label>
            <input type="text" name="ia" value="<?= htmlspecialchars($p['ia_destino']) ?>" required>

            <label>Contenido</label>
            <textarea name="contenido" rows="8" required><?= htmlspecialchars($p['contenido']) ?></textarea>

            <div class="btn-row">
                <button type="submit" class="guardar-btn">Guardar cambios</button>
                <a href="edit.php" class="cancelar-btn">Cancelar</a>
            </div>
